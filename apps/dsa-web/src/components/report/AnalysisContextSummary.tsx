...
const MISSING_REASON_LABELS: Record<ReportLanguage, Record<string, string>> = {
  zh: {
    ...
    news_context_missing: '新闻未进入本次分析，结论未使用新闻上下文；请检查搜索配置、网络或限流后重新分析',
    ...
    fundamental_source_chain_missing: '未找到可用的基本面来源链，本次分析未使用基本面数据；请检查数据源配置后重新分析',
  ...
  en: {
    ...
    news_context_missing: 'News was not included, so the conclusion did not use news context; check search configuration, network, or rate limits and rerun',
    ...
    fundamental_source_chain_missing: 'No usable fundamental source chain was found, so fundamental data was not used; check the data-source configuration and rerun',
  ...
  ko: {
    ...
    news_context_missing: '뉴스가 포함되지 않아 결론에 뉴스 맥락이 반영되지 않았습니다. 검색 설정, 네트워크 또는 제한을 확인한 후 다시 분석하세요',
    ...
    fundamental_source_chain_missing: '사용 가능한 펀더멘털 소스 체인을 찾지 못해 분석에 사용되지 않았습니다. 데이터 소스 설정을 확인한 후 다시 분석하세요',
