...
このフォークは `NVDA`、米国市場レビュー、英語レポートに設定済みです。GitHub Actions は平日 19:00（日本時間）に実行され、Actions タブからも手動実行できます。
...
分析が成功すると、最新レポートのサマリーだけが GitHub Pages に公開されます。個別の詳細レポート、Actionsログ、メールアドレス、認証情報は公開しません。サマリーの形式を認識できない場合も、詳細を代わりに公開せず、Pagesの更新をスキップします。
