...
- 日本語で回答してください。
- JSONを出力する場合、キーは変更せず、人が読むすべての値を日本語にしてください。
"""
        return """
## 输出语言
...
- JSONキーは変更しないでください。
- `decision_type` は `buy|hold|sell` のいずれかを維持してください。
- `stock_name`、`trend_prediction`、`operation_advice`、`confidence_level`、ダッシュボード本文、チェック項目、要約を含む、人が読むすべての値を日本語で記述してください。
"""
    ...
    def _build_user_message(self, task: str, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        if context:
            ...
            if report_language == "en":
                ...
                parts.append("出力言語: 日本語（JSONキーは変更せず、人が読むすべての値を日本語で記述）")
            else:
